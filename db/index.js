...
const client = new Client({
    ...
    ssl: process.env.NODE.ENV === 'production' ? { rejectUnauthorized: false } : undefined,
});
...
const createPost = async ({authorId, title, content, tags=[]}) => {
    try {
        ...
        const tagArray = await createTags(tags);
        const result = await addTagsToPost(post.id, tagArray);
...
const getAllTags = async () => {
    ...
    try {
        ...
        console.error("There was an error getting all the tags", error);
        throw error;
    }
}
